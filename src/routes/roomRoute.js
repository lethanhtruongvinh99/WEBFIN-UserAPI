...
const { createNewRoom, findRoomById, addNewMemberToRoom, getAllRoom, getRoomDetail, addMoveToRoom, addRoomPlayerB, createQuickPlayRoom, getGameStats } = require("../controllers/rooms.controller");
...
router.get("/gamestats", async (req, res) => {
  passport.authorize("jwt", { session: false }, async (err, user, info) => {
    ...
    if (info) {
      ...
      const gameStats = await getGameStats(user._id);
      if (gameStats.status) {
        return res.status(200).json({ totalGame: gameStats.totalGames, winGame: gameStats.winGame });
      } else {
